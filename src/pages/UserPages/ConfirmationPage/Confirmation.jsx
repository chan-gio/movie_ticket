...
function Confirmation() {
  ...
  const handleDownloadBarcode = () => {
    if (barcodeUrl) {
      ...
      link.download = `ticket_barcode_${bookingId}.png`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      message.success('Barcode downloaded successfully');
    } else {
      message.error('Barcode not available for download');
    }
  };
  ...
  return (
    <div className={styles.confirmation}>
      <Card className={styles.mainCard}>
          ...
          <Row>
            <Col xs={24} md={16} className={styles.leftTicket}>
              ...
              <div className={styles.ticketBody}>
                ...
                <div className={styles.barcodeContainer}>
                  {barcodeUrl ? (
                    <img src={barcodeUrl} alt="Ticket Barcode" className={styles.barcode} />
                  ) : (
                    <canvas
                      ref={barcodeRef}
        ...
        <Space className={styles.buttons}>
          <Button icon={<DownloadOutlined />} className={styles.actionButton} onClick={handleDownloadBarcode}>
            Download Barcode
          </Button>
          <Button icon={<PrinterOutlined />} className={styles.actionButton} onClick={handlePrint}>
            Print
          </Button>
        </Space>
      </Card>
